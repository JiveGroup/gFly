...
func TestValidate(t *testing.T) {
	// Define a simple struct for validation
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// Additional check for error existence when validation fails
			if !test.expected && err == nil {
				t.Errorf("Expected validation to return an error")
			}
		})
	}
}
